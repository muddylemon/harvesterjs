...
module.exports = function (harvesterApp) {
    ...
    return function (oplogMongodbUri) {
        ...
        EventsReader.prototype.tail = function() {
            ...
            var that = this;
            var Checkpoint = harvesterApp.adapter._models['checkpoint'];
                return Checkpoint.findOne({}).exec()
                .then(function (checkpoint) {
                    if (!checkpoint) {
                        debug('checkpoint missing, creating... ');
                        return Checkpoint.create({ts: BSON.Timestamp(0, Date.now() / 1000)})
                            .then(function () {
        ...
        EventsReader.prototype.updateCheckpointAndReschedule = function(doc) {
            ...
            if (doc != null) {
                ...
                if (!matchCheckpoint) {
                    ...
                    var Checkpoint = harvesterApp.adapter._models['checkpoint'];
                    return Checkpoint.findByIdAndUpdate(that.checkpoint.id, {ts: ts}).exec()
                        .then(function () {
                            that.reschedule(0);
                        });
