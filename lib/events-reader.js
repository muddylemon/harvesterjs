...
module.exports = function (harvestApp) {
    ...
    return function (oplogMongodbUri) {
        ...
        EventsReader.prototype.tail = function() {
            ...
            if (this.stopped) throw new Error('can not tail a ');
            // todo quick hack to prevent reuse of existing events-reader,
            // right now it will cause issues because mongojs tailable cursor stream doesn't support end()

            this.stopped = Promise.defer();

            var that = this;

            return harvestApp.adapter.find('checkpoint', {})
                ...
                .catch(function (err) {
                    if (!err) {
                        ...
                        harvestApp.adapter.create('checkpoint', {ts: BSON.Timestamp(0, Date.now() / 1000)})
                            .then(function () {
                                tail();
                            });
                    } else {
        ...
        EventsReader.prototype.processDocHandlers = function(doc) {
            ...
            if (doc != null) {
                ...
                _.forOwn(harvestApp.changeHandlers, function (changeHandlers, key) {
        ...
        EventsReader.prototype.updateCheckpointAndReschedule = function(doc) {
            ...
            if (doc != null) {
                ...
                if (!matchCheckpoint) {
                    ...
                    return harvestApp.adapter.update('checkpoint', that.checkpoint.id, {ts: ts})
                        .then(function () {
                            that.reschedule(0);
                        });
