...
SSE.prototype.handler = function (req, res, next) {
    ...
    .then(function (query) {
            ...
            stream.on('data', function (chunk) {
                if(chunk.ns === 'fuse-telemetry.checkpoints') return;
                debug('chunk is ', chunk)
                ...
                debug('resource names ', resourceNames, chunk.ns)
                
                var matchesEitherResource = _.some(resourceNames, function(resourceName) {
                    return resourceName.test(chunk.ns);
                });
                
                debug('resource names ', resourceNames, chunk.ns, matchesEitherResource)
            ...
            stream.on('end', function () {
                console.log('mongodb stream ended');
                res.end();
            });
            stream.on('err', function (e) {
                console.error('mongodb stream error ' + e);
                res.end();
            });
            res.on('close', function () {
                console.error('response stream is closed, destroy mongodb stream');
                stream.destroy();
            });
        })
        .catch(function (err) {
...
SSE.prototype.getQuery = function(lastEventId) {
        debug('lastEventId is ', lastEventId)
        var collection = this.db.collection('oplog.rs')
        return new Promise(function (resolve, reject) {
            ...
            collection.find({}, {sort : {$natural : -1}, limit : 1}).each(function (err, item) {
                    if (err) {
                        reject('mongodb stream error ' + err)
                    }
                    if (item) {
                        var query = {};
                        query.ts = {
                        ...
                        reject('no item found matching oplog query')
                    }
                })
        })
    }
