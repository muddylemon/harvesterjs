...
function route(name, model, routeOptions) {
    ...
    */
    ...
    function findResources(type, baseUri, ids) {
        if (!baseUri) {
            var adapter = this.adapter,
            ...
            // use http to fetch and strip down for consumption
            var relatedResource = inflect.pluralize(type);
            return $http(baseUri + '/' + relatedResource + '?id='+ids.join(','), {json: true})
                .spread(function (response, body){
                    return body[relatedResource];
                })
        }
    }
    ...
    function getLinked(fetchedIds, resources, schema, key, req, res) {
        ...
        if (ids.length > 0) {
            ...
            return findResources.call(this,type,baseUri,ids).then(function(resources) {
                return RSVP.all(resources.map(function(resource) {
