...
function route(name, model, routeOptions) {
            ...
            .then(function (linkedResources) {
                ...
                if (Object.keys(linkedResources).length) {
                    ...
                    primaryResources.forEach(function (resource) {
                        promises.push(name.findOne(resource.id).exec());
                    });
                    ...
                    RSVP.all(promises).then(function (resources) {
                        ...
                        body[collection] = resources.map(deserialize);
                ...
                .then(function (resources) {
                    return RSVP.all(resources.map(function (resource) {
                        ...
                        return model.create(resource)
                            .then(function(resp){
                                resp = resp.toObject({virtuals: true});
                                delete resp._id;
                                delete resp.__v;
                                return resp;
                            },function(err){
    ...
    this.fnHandlers[individualRouteResource]["get"] = function (req, res) {
        ...
        findMany(model, id)
            ...
            .then(function (resource) {
                ...
                return findMany(relatedModel, ids)
                    ...
                    .then(function (resources) {
                        ...
                        body[relatedKey] = resources.map(deserialize);
                        sendResponse(req, res, 200, body);
                    });
            ...
            .then(function () {
                ...
                beforeTransform(update, req, res)
                    ...
                    .then(function (update) {
                        return model.findByIdAndUpdate(id, update).exec();
                    }, sendError)

                    // do after transform
                    .then(function (update) {
                        update = update.toObject({virtuals: true});
                    ...
                    .then(function (update) {
                        ...
                        body[collection] = [deserialize(update)];
                        sendResponse(req, res, 200, body);
                    }, sendError);
            },
            ...
            function (error) {
                ...
                if (!!error) {
                    ...
                    beforeTransform(update, req, res)
                        ...
                        // create the resource
                        .then(function (resource) {
                            return create(model, id, resource);
                        }, catchError)
    ...
    this.fnHandlers[individualRoute]["delete"] = function (req, res) {
        ...
        model.findById(id).exec()
            ...
            // let's delete it
            .then(function () {
                return model.findByIdAndRemove(id).exec().then(function () {
    ...
    */
    ...
    function findResources(type, ids) {
        var model = models[inflect.singularize(type)];
        return findMany(model, ids);
    }
        ...
        if (ids.length > 0) {
            ...
            return findResources.call(this,type,ids).then(function(resources) {
                return RSVP.all(resources.map(function(resource) {
                    ...
                    return {type: inflect.pluralize(type), resources: resources.map(deserialize)};
                });
            });
        }
...
function serialize(resource) {
    resource.id && (resource._id = mongoose.Types.ObjectId(resource.id.toString()));
    return resource;
}

function deserialize(resource) {
    resource = resource.toObject ? resource.toObject({virtuals: true}) : resource;
    delete resource._id;
    delete resource.__v;
    return resource;
}

function findMany(model, query, limit, skip, sort, fields) {
    if(_.isObject(query)){
    ...
    skip = skip ? skip : 0;
    sort = sort || {"_id":-1};
    return model.find(query).skip(skip).sort(sort).limit(limit).select(fields).exec();
}
