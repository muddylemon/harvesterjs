...
Harvester.prototype._init = function (options) {
  ...
  if (options.authFunction) {
    ...
    router.use(function(req, res, next) {
      var resourceRegexp = /\/([a-z]*)/g;
      var name = resourceRegexp.exec(req.path);


      if(!(name && name[1])) {
        return options.authFunction(req)
        .then(next)
        .catch(function() {
          res.send(401)
        });
      ...
      name = inflect.singularize(name[1]);
      var route = createdRoutes[name];
      if (!routeOptions[name]) return next();

      if (routeOptions[name].auth === false) {
        next();
      } else if(route) {
        return options.authFunction(req)
        .then(next)
