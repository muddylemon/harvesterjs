 ...
 * @return {this}
 */
Harvester.prototype.resource = function (name, schema, options) {
  var _this = this;
  options = options || {};
  ...
  this._dsl = _.clone(schema);
  schema = this._convertToLegacySchema(schema);
  schema = this._preprocessSchema(schema);
  // Store a copy of the input.
  this._schema[name] = _.clone(schema);
  this.changeHandlers[name] = [];
  options.authFunction = this.authFunction;

  schema = _this.adapter.schema(name, schema, options);
  createdRoutes[name]=_this._route(name, _this.adapter.model(name, schema), options);
...
Harvester.prototype.setAuthorizationHandler = function(authFunction) {

  this.authFunction = authFunction;


  //if route is created before the auth handler is added, make sure it's passed over
  _.each(createdRoutes, function(context, route) {
    console.log(createdRoutes[route].authFunction)
    createdRoutes[route].authFunction = authFunction;
  });
};
