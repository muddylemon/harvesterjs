 ...
 *                 this can include instance functions, via options.methods
 *                 and static functions via options.statics
 * @return {this}
 */
Harvester.prototype.resource = function (name, schema, options) {
  ...
  // peel off the options "hitchhiker" objects, if any
  if (options) {
    if (options.methods) {
      methods = options.methods;
      options.methods = undefined;  // reset this like it was never there
    }
    if (options.statics) {
        statics = options.statics;
        options.statics = undefined; // reset this like it was never there
    }
  }
  ...
  // add instance functions to the schema, if any
  if (methods) {
    schema.methods = methods;
  }
  // add static function to the schema, if any
  if (statics) {
