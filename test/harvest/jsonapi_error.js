...
module.exports = function(baseUrl,keys,ids) {
    describe('jsonapi error handling', function () {
        ...
        describe('bulk insert 2 entities into /foobars resource, each before callback rejects with an JSONAPI_Error', function () {
            it('should respond with a 400, content-type set to application/vnd.api+json and the errors object carries the first error encountered', function (done) {
                request(baseUrl)
                    .post('/foobars')
                    .send({foobars: [
                        {foo: 'bar'},
